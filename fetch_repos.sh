...
while true; do
  ...
  page_repos=$(echo "$repos" | jq '[.[] | select(.has_pages or (.name | contains("io"))) | {name: .name, url: ("https://" + .owner.login + ".github.io/" + .name), github_url: .html_url, description: (.description // ""), pushed_at: .pushed_at}]')
